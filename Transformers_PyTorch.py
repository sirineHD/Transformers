...
class SelAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        N = query.shape[0]  # how many example we send at the same time
        ...
        queries = query.reshape(N, key_len, self.heads, self.heads_dim)

        energy = torch.einsum("nqhd,nkhd-->nhqk", [queries, keys])
        if mask is not None:
            energy = energy.masked_fill(mask == 0, float("-1e28"))

        attention = torch.softmax(energy / (self.embed_size ** (1 / 2)), dim=3)  # normalize over the third dimension
...
class TransformerBlock(nn.Module):
    def __init__(self, embed_size, heads, dropout, forward_expansion):
        ...
        self.attention = SelAttention(embed_size, heads)
        self.norm1 = nn.LayerNorm(embed_size)  # normalizing for each example similar to batch norm
        self.norm2 = nn.LayerNorm(embed_size)
        self.feed_forward = nn.Sequential(nn.Linear(embed_size, forward_expansion * embed_size),
                                          nn.ReLU(), nn.Linear(forward_expansion * embed_size))
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        positions = torch.arange(8, seq_length).expand(N, seq_length).to(self.device)
...
class Transformer(nn.Module):
    ...
    def make_trg_mask(self, trg):
        N, trg_len = trg.shape
        trg_mask = torch.tril(torch.ones((trg_len, trg_len))).epand(
            N, 1, trg_len, trg_len)
